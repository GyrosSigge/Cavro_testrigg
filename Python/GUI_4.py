...
class MotorPanel(ttk.Frame):
    ...
    def update_graph(self):
        ...
        self.position_line.set_data(self.time_points, self.position_points)
         ...
         # Instead of relim/autoscale, just update x-limits
        if self.time_points:
            self.ax_pressure.set_xlim(self.time_points[0], self.time_points[-1])

            # Compute min/max of the current window
            min_val = min(self.pressure_points)
            max_val = max(self.pressure_points)
            self.pressure_recent_max.set(f"{max_val:.1f}")

            # Only update if the limits have changed
            ymin, ymax = self.ax_pressure.get_ylim()
            new_ymin = min(0, min_val)
            new_ymax = max_val * 1.2
            if new_ymin != ymin or new_ymax != ymax:
                self.ax_pressure.set_ylim(new_ymin, new_ymax)

            # Repeat for other axes
            min_val = min(self.current_points)
            max_val = max(self.current_points)
            self.current_recent_max.set(f"{max_val:.1f}")
            ymin, ymax = self.ax_current.get_ylim()
            new_ymin = -0.5
            new_ymax = max_val * 1.2
            if new_ymin != ymin or new_ymax != ymax:
                self.ax_current.set_ylim(new_ymin, new_ymax)

            min_val = min(self.position_points)
            max_val = max(self.position_points)
            self.position_recent_max.set(f"{max_val:.1f}")
            ymin, ymax = self.ax_position.get_ylim()
            new_ymin = -0.5
            new_ymax = max_val * 1.2
            if new_ymin != ymin or new_ymax != ymax:
