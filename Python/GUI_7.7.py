...
cavro_cmd_queue = queue.Queue() # NEW: central Cavro command queue
...
class SerialCavroWorker(QtCore.QThread):
    plunger_signal = QtCore.pyqtSignal(int, float)
    valve_signal   = QtCore.pyqtSignal(int, int)
        ...
        while self._running:
            try:
                if ser_cavro.is_open:
                    # 1. Process queued GUI commands first
                    try:
                        while True:
...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def btn_set_freq_func(self):
        if ser_esp32.is_open:
            ...
            print("ESP32 serial port not open.")
...
if __name__ == "__main__":
    ...
    if ser_esp32.is_open:
        try:
            ...
            ser_esp32.read_until()  # Clear buffer
            ser_esp32.write(msg.encode())
            print(msg)
        except ValueError:
            print("Invalid frequency value. Please enter an integer.")
    ...
    if ser_cavro.is_open:
        try:
            ...
            print("Failed to init. all motors.")
