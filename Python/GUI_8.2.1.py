...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def btn_loop_all_func(self):
        ...
        message = ""
        for i in range(NUM_MOTORS):
            message += f"/{ADDRESS[i]}Z,g,M500,O,M500,A{self.start_pos},M500,I,M500,A{self.stop_pos},G{self.loop_cycles},M500,O,M500,A{self.start_pos}R,"
        send_message(None, message, RAW=True)
...
if __name__ == "__main__":
    ...
    if ser_cavro.is_open:
        try:
            for i in range(NUM_MOTORS):
                cavro_cmd_queue.put(f"/{ADDRESS[i]}ZR\r")
        except Exception:
            pass
