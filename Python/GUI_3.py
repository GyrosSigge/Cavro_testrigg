...
MAX_POINTS = 2000  # keep last N points
...
class MotorPanel(ttk.Frame):
    def __init__(self, parent, motor_id, data):
        ...
        ttk.Button(controls, text="Valve Input", width=13, command=self.valve_set_input).grid(row=1, column=1, columnspan=1, pady=2, padx=2)
        ttk.Button(controls, text="Valve Output", width=13, command=self.valve_set_output).grid(row=2, column=0, columnspan=1, pady=2, padx=2)
        ttk.Button(controls, text="Valve Extra", width=13, command=self.valve_set_extra).grid(row=2, column=1, columnspan=1, pady=2, padx=2)
        ...
        fig.subplots_adjust(left=0.2, bottom=0.2, right=0.8, top=0.99)
    ...
    def valve_set_input(self): send_message(self.motor_id, f"IR")
    def valve_set_output(self): send_message(self.motor_id, f"OR")
    def valve_set_extra(self): send_message(self.motor_id, f"ER")
