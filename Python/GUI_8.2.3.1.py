...
for i in range(1, NUM_MOTORS+1):
    header += [f'Motor{i}_Pressure', f'Motor{i}_Plunger_Position', f'Motor{i}_Valve_Position']
...
class SamplingWorker(QtCore.QThread):
    ...
    def __init__(self, sample_freq:float=50.0, parent=None):
        super().__init__(parent)
    ...
    def run(self):
        # loop and sample latest motor_data + Fluigent pressures
        ...
        while self._running:
            ...
            # read latest positions & valves from motor_data (thread-safe)
            ...
            # push latest sample into data_queue (overwrite if necessary)
            try:
            ...
            # write CSV asynchronously here (keeps sampling decoupled from GUI)
            row = [timestamp.strftime('%Y-%m-%d %H:%M:%S.%f')]
            for i in range(NUM_MOTORS):
                row += [pressures[i], positions[i], valves[i]]
            with log_lock:
            ...
            if self._last_time is None:
                ...
                if dt > 0:
                    ...
                    if len(freq_samples) > max_samples:
                        freq_samples.pop(0)  # keep window size bounded
            ...
            # sleep to maintain sample rate (account for time spent)
...
class MotorPanel(QtWidgets.QWidget):
    ...
    def update_valve_position(self, valve_p: int):
        with motor_data_lock:
            motor_data[self.motor_id]['valve'] = valve_p
        if valve_p == 1:
...
class MainWindow(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        self.sampler = SamplingWorker(sample_freq=self.sampling_freq)
    ...
    def btn_loop_all_func(self):
        try:
            ...
            print("Invalid pressure/pos/cycles value. Please enter integers.")
            return
        message = f"/_Z,g,M500,O,M500,A{self.start_pos},M500,I,M500,A{self.stop_pos},G{self.loop_cycles},M500,O,M500,A{self.start_pos}R"
        send_message(None, message, RAW=True)
