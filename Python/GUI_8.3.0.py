...
# ------------------ CONFIG ------------------
STROKE_MM = 40
MAX_POINTS = 500  # max points per plot
NUM_MOTORS = 6
...
for i in range(1, NUM_MOTORS+1):
    header += [f'Motor{i}_Pressure', f'Motor{i}_Pressure_Flag', f'Motor{i}_Plunger_Position', f'Motor{i}_Valve_Position']
...
motor_data = [{"pressure":0.0,"position":0.0,"valve":0} for _ in range(NUM_MOTORS)]
        ...
        while self._running:
            try:
                if ser_cavro.is_open:
                    ...
                    # Read plunger for each motor (fast)
...
# ------------------ SAMPLING WORKER (moved out of GUI) ------------------
class SamplingWorker(QtCore.QThread):
    freq_signal = QtCore.pyqtSignal(float)  # emit actual sampling frequency

    def __init__(self, sample_freq:float=50.0, max_pressure=7000, parent=None):
        super().__init__(parent)
        self.sample_freq = sample_freq
        self.max_pressure = max_pressure
        self._running = True
        self._period = 1.0 / max(self.sample_freq, 1.0)
        self._last_time = None
        self.flags = [0]*NUM_MOTORS
    ...
    def run(self):
        ...
        freq_samples = []              # store recent dt values
        max_samples = 50               # adjust window size as needed

        # Track when each motor pressure first goes low
        low_start_times = [None] * NUM_MOTORS  
        needs_restart = [False] * NUM_MOTORS   # Track if motor actually needs a restart
        ...
        while self._running:
                ...
                try:
                    ...
                    if sensor_id in sensor_map:
                        ...
                        if idx < NUM_MOTORS:
                            ...
                            motor_data[idx]['pressure'] = pressures[idx]

                            # --- Safety: stop if pressure too high ---
                            if pressures[idx] >= self.max_pressure:
                                if self.flags[idx] == 0:
                                    send_message(idx, "T")   # stop motor
                                    send_message(idx, "T")
                                    self.flags[idx] = 1
                                    low_start_times[idx] = None
                                    needs_restart[idx] = True   # mark for restart
                                    print(f"Warning Motor{idx+1} exceeded max pressure, P={pressures[idx]}")
                            else:
                                self.flags[idx] = 0
                                # Only consider restart if motor was stopped for high pressure
                                if needs_restart[idx]:
                                    if low_start_times[idx] is None:
                                        low_start_times[idx] = time.time()
                                    elif (time.time() - low_start_times[idx]) >= 1.0:
                                        # Low for at least 0.5s → restart motor
                                        start_message = (
                                            f"Z,g,M500,E,M500,A{w.start_pos},M500,I,"
                                            f"M500,A{w.stop_pos},G{w.loop_cycles},M500,E,"
                                            f"M500,A{w.start_pos}R,"
                                        )
                                        send_message(idx, start_message)
                                        print(f"Restarting Motor {idx+1} after safe pressure")
                                        low_start_times[idx] = None
                                        needs_restart[idx] = False  # reset after restart
                except Exception as e:
                    print(f"Error reading Fluigent sensor {sensorInfo.index}: {e}")

            # --- rest of your code unchanged (positions, queue, logging, freq calc) ---
            with motor_data_lock:
                positions = [motor_data[i]['position'] for i in range(NUM_MOTORS)]
                valves    = [motor_data[i]['valve'] for i in range(NUM_MOTORS)]
            ...
            for i in range(NUM_MOTORS):
                row += [pressures[i], self.flags[i], positions[i], valves[i]]
            with log_lock:
            ...
            if self._last_time is None:
                ...
                if dt > 0:
                    ...
                    if len(freq_samples) > max_samples:
                        freq_samples.pop(0)
                    if (time.time() - freq_update_start) >= 1.0:
                        freq_update_start = time.time()
...
class MotorPanel(QtWidgets.QWidget):
    ...
    def update_plot(self):
        ...
        with motor_data_lock:
            p = motor_data[self.motor_id]['pressure']
            pos = motor_data[self.motor_id]['position']
            valve = motor_data[self.motor_id]['valve']
        self.status_pressure.setText(f"Pressure: {p:.1f}")
        self.status_position.setText(f"Position: {pos:.1f}")
...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def on_plunger_update(self, motor_id:int, raw_pos:float):
        # update motor_data in a thread-safe way
        pos_mm = - raw_pos / 3000.0 * STROKE_MM
        with motor_data_lock:
            motor_data[motor_id]['position'] = pos_mm

    def on_valve_update(self, motor_id:int, valve_p:int):
        with motor_data_lock:
            motor_data[motor_id]['valve'] = valve_p
    ...
    def btn_loop_all_func(self):
        try:
            ...
            print("Invalid pressure/pos/cycles value. Please enter integers.")
            return
        message = f"/_Z,g,M500,E,M500,A{self.start_pos},M500,I,M500,A{self.stop_pos},G{self.loop_cycles},M500,E,M500,A{self.start_pos}R"
        send_message(None, message, RAW=True)
    ...
    def update_all_panels(self):
        ...
        try:
            ...
            # no new sample; just refresh status from motor_data
            pressures = None
            positions = None
            valves = None
            timestamp = datetime.now()
