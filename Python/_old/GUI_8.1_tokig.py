...
class MotorPanel(QtWidgets.QWidget):
    ...
    def btn_loop_func(self):
        try:
            ...
            return
        message = f"Z,g,M500,O,M500,A{w.start_pos},M500,I,M500,A{w.stop_pos},G{w.loop_cycles},M500,O,M500,A{w.start_pos}R,"
...
class MainWindow(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        self.entry_set_freq = QtWidgets.QLineEdit("25")
        ...
        self.sampling_worker = SamplingWorker(self.sampling_freq)
        self.sampling_worker.data_signal.connect(self.on_sample_data)
    ...
    def on_sample_data(self, pressures, positions, timestamp):
        ...
        if self._last_sample_time is not None:
            ...
            if dt > 0:
                ...
                self.freq = alpha * (1.0 / dt) + (1 - alpha) * self.freq
    ...
    def btn_loop_all_func(self):
        ...
        message = ""
        for i in range(NUM_MOTORS):
            message += f"/{i+2}Z,g,M500,O,M500,A{self.start_pos},M500,I,M500,A{self.stop_pos},G{self.loop_cycles},M500,O,M500,A{self.start_pos}R,"
        send_message(None, message, RAW=True)
    ...
    def btn_set_freq_func(self):
        try:
            ...
            # Update the sampling worker with the new frequency
            self.sampling_worker.set_sampling_freq(freq_local)
            print(f"Sampling freq set to {self.sampling_freq} Hz")
        except Exception as e:
            print("Invalid frequency value.", e)
    ...
    def closeEvent(self, event: QtGui.QCloseEvent):
        try:
            if hasattr(self, 'sampling_worker'):
...
class SamplingWorker(QtCore.QThread):
    data_signal = QtCore.pyqtSignal(list, list, datetime)  # pressures, positions, timestamp

    def __init__(self, sampling_freq: float, parent=None):
        super().__init__(parent)
        self.sampling_freq = sampling_freq
        self._running = True
        self.csv_buffer = []
        self.csv_buffer_size = 1
        self._interval = 1.0 / max(sampling_freq, 1)
        self._sleep_time = max(0.0001, self._interval * 0.1)  # adaptive sleep time

    def run(self):
        # Pre-allocate arrays to avoid memory allocation during sampling
        pressures = [0.0] * NUM_MOTORS
        positions = [0.0] * NUM_MOTORS
        
        # Use monotonic clock for better timing precision
        next_time = time.perf_counter()
        
        while self._running:
            if hasattr(self, '_next_time_reset') and self._next_time_reset:
                next_time = time.perf_counter()
                self._next_time_reset = False

            # Busy-wait with adaptive sleep for precise timing
            now = time.perf_counter()
            while now < next_time - self._sleep_time:
                time.sleep(self._sleep_time)
                now = time.perf_counter()
            
            # Fine-grained busy wait for the last microsecond
            while now < next_time:
                now = time.perf_counter()
            
            next_time += self._interval  # Use additive approach for better stability
            
            try:
                # -------- Sample pressures --------
                for i in range(NUM_MOTORS):
                    pressures[i] = 0.0  # Reset before sampling
                ...
                # -------- Get positions from motor_data --------
                for i in range(NUM_MOTORS):
                    positions[i] = motor_data[i]['position']

                # -------- Timestamp --------
                timestamp = datetime.now()

                # -------- Emit data to GUI / queue --------
                self.data_signal.emit(pressures.copy(), positions.copy(), timestamp)
                ...
                if len(self.csv_buffer) >= self.csv_buffer_size:
                    log_writer.writerows(self.csv_buffer)
                    log_file.flush()
                    self.csv_buffer.clear()
                    
            except Exception as e:
                print(f"Error in sampling loop: {e}")
                # Continue running despite errors to maintain timing

    def set_sampling_freq(self, new_freq):
        """Update sampling frequency and recalculate timing parameters"""
        with threading.Lock():  # Add a lock if you experience timing issues
            self.sampling_freq = new_freq
            self._interval = 1.0 / max(new_freq, 1)
            self._sleep_time = max(0.0001, self._interval * 0.1)
    ...
    def stop(self):
        ...
        if self.csv_buffer:
            try:
                ...
                print(f"Error flushing final CSV data: {e}")
...
if __name__ == "__main__":
    ...
    if ser_cavro.is_open:
        try:
            for i in range(NUM_MOTORS):
                cavro_cmd_queue.put(f"/{i+2}ZR\r")
        except Exception:
            pass
