...
import sys, queue, threading, time, serial, csv, os
...
for i in range(1, NUM_MOTORS+1):
    header += [f'M{i}_Pressure', f'M{i}_Pressure_Flag', f'M{i}_Plunger_Position', f'M{i}_Valve_Position']
...
# ------------------ SAMPLING WORKER (time-based commit scheduling + error capture) ------------------
class SamplingWorker(QtCore.QThread):
    ...
    def __init__(self, sample_freq: float = 10.0, max_pressure=7000, parent=None):
        ...
        self._running = True
        self.flags = [False] * NUM_MOTORS
        self.set_rate(sample_freq)  # ensure clamping and setup

        # --- Error capture setup ---
        self.error_buffer = deque(maxlen=50 * 5)   # keep last 5 sec of data at 50Hz
        self.error_active = [False] * NUM_MOTORS   # whether currently capturing after trigger
        self.error_capture = [[] for _ in range(NUM_MOTORS)]
        self.error_after_samples = int(50 * 2)     # capture 2 sec after trigger
        self.error_remaining = [0] * NUM_MOTORS
            ...
            for i in range(NUM_MOTORS):
                if not self.error_active[i]:
                    ...
                    if self.error_remaining[i] <= 0:
                        # Save to CSV
                        fname = f"error_motor{i+1}_{timestamp.strftime('%Y%m%d_%H%M%S')}.csv"
                        with open(fname, "w", newline="") as f:
                            writer = csv.writer(f)
                            header = ['Timestamp']
                            for j in range(1, NUM_MOTORS+1):
                                header += [f'M{j}_Pressure', f'M{j}_Pressure_Flag', f'M{j}_Plunger_Position', f'M{j}_Valve_Position']
                            writer.writerow(header)
                            writer.writerows(self.error_capture[i])
                        print(f"⚠️ Error data saved: {fname}")
                        # Reset
